...
def train(
    ...
    # ========================================================
    # INITIAL-ONLY CHECKPOINT SAVE + EXIT
    # ========================================================
    # 이 스크립트는 학습 전 상태(Epoch 0)를 공정하게 평가하기 위한 전용 복사본이다.
    # Q2 Event K-means 초기화와 학습 전 Train/Validation loss 계산까지 수행한 뒤,
    # optimizer update를 한 번도 하지 않은 모델을 checkpoint_initial.pt로 저장하고 종료한다.
    ...
    if accelerator.is_main_process:
        ...
        initial_state["initial_loss_snapshot"] = {
            "train": initial_train_result,
            "validation": initial_eval_result,
            "gumbel_temperature": initial_gumbel_t,
            "note": "Before any optimizer update; after fresh Q2 Event K-means initialization.",
        }
        ...
        safe_torch_save(initial_state, initial_path)

        print("\n" + "=" * 70)
        print(f"Initial model saved: {initial_path}")
        print("Optimizer updates: 0")
        print("Training loop skipped intentionally.")
        print("=" * 70 + "\n")
