...
# STEP 10-4. Validation-origin event의 고정 z(E) 생성
def build_validation_event_embeddings() -> dict[str, Any]:
    """
    validation에서 새로 생성된 event(envet_origin_split == "validation")의 
    event embedding z(E) 생성

    event_id | event_origin_split | first_article_id | event_article_count 

    정책
    ----------------------------
    1. Train-origin Event 
        - 이미 train에서 z(E)가 생성되어 있기에 여기서 다시 만들지 않는다
        - Validation 기사가 추가되더라도 train z(E)는 갱신하지 않는다
    
    2. Validation-origin event
        - event가 처음 생성된 시점의 최초 기사(first_article_id)의 
        E5 article embedding을 z(E)로 사용한다
        - 이후 같은 Event에 다른 Validation 기사가 추가되더라도
        z(E)는 다시 평균내거나 갱신하지 않는다.

    즉 : Validation 기사 V1 -> 새로운 event E 생성 -> z(E) = embedding(V1)
    -> 이후 V2,V3가 E에 들어와도 z(E)는 embedding(V1) 그대로 유지 

    이 단계에서는 c2 계산하지 않고, 이후 RQ-VAE 학습 완료 후:
    z(E) -> 학습된 Q2 codebook vectors와 argmin -> c2 결정 
    ...
    # STEP 10-4-2. 
    # Validation까지 반영된 event master 읽기 
    event_master = pl.read_parquet(config.EVENT_MASTER_WITH_VALIDATION_PATH).\
    select([
        "event_id",
        "event_origin_split",
        "first_article_id",
    ...
    # STEP 10-4-3. Validation에서 새롭게 생성된 event만 선택
    # Train-origin event는 이미 train_event_embeddings.npy에 z(E)가 존재하므로 제외
    validation_events = event_master.filter(pl.col("event_origin_split")=="validation").\
        select(["event_id", "first_article_id", "event_article_count"]).sort("event_id")

    validation_event_count = validation_events.height 
    # Event ID 중복 검사
    duplicate_event_id_count = validation_events.select(
        pl.col("event_id").is_duplicated().sum()
    ).item()

    if duplicate_event_id_count != 0:
        raise ValueError(
            "Validation-origin Event에 중복 event_id가 존재합니다. "
            f"중복 행 수 = {duplicate_event_id_count}"
        )

    # 최초 기사 ID null 검사
    first_article_null_count = validation_events.get_column("first_article_id").null_count()

    if first_article_null_count != 0:
        raise ValueError( "Validation-origin Event의 first_article_id에 "
            "null 값이 존재합니다. "
            f"null 개수={first_article_null_count}")

    # STEP 10-4-4. article_id - embedding_row 매핑 읽기
    article_embedding_input = pl.read_parquet(config.ARTICLE_EMBEDDING_INPUT_PATH).select([
    ...
    # article_id는 하나의 embedding_row만 가져야함
    duplicate_article_id_count = article_embedding_input.select(
        pl.col("article_id").is_duplicated().sum()
    ).item()

    if duplicate_article_id_count != 0:
        raise ValueError(
            "article_embedding_input.parquet에 "
            "중복 article_id가 존재합니다. "
            f"중복 행 수 = {duplicate_article_id_count}"
        )

    # STEP 10-4-5. Validation-origin event의 first_article_id에 embedding_row 연결
    validation_event_embedding_input = validation_events.join(
        article_embedding_input, 
        left_on="first_article_id",
        right_on="article_id",
        how="left",
    ).select([
        "event_id", "first_article_id", "event_article_count", "embedding_row",
    ]).sort("event_id")

    # 모든 first_article_id가 embedding_row 가져야함
    missing_embedding_row_count = validation_event_embedding_input.get_column("embedding_row").null_count()

    if missing_embedding_row_count != 0:
        missing_examples = validation_event_embedding_input.filter(
            pl.col("embedding_row").is_null()
        ).select([
            "event_id", "first_article_id",
        ]).head(10).to_dicts()

        raise ValueError(
            "Validation-origin Event의 최초 기사 중 "
            "Article Embedding을 찾을 수 없는 기사가 있습니다. "
            f"누락 수={missing_embedding_row_count}, "
            f"예시={missing_examples}"
        )
    # STEP 10-4-6. 전체 Article E5 Embedding 읽기 
    ...
    article_embedding_row_count = int(
        article_embeddings.shape[0]
    )

    article_embedding_dimension = int(
        article_embeddings.shape[1]
    )

    # STEP 10-4-7. embedding_row 범위 검사 
    if validation_event_count > 0:

        minimum_embedding_row = int(
            validation_event_embedding_input
        ...
        if minimum_embedding_row < 0:
            raise ValueError(
                "Validation Event embedding_row에 "
                "음수가 존재합니다. "
                f"최솟값={minimum_embedding_row}"
            )

        if maximum_embedding_row >= article_embedding_row_count:
            raise ValueError(
                "Validation Event embedding_row가 "
                "article_embeddings.npy 범위를 벗어났습니다. "
                f"최대 row={maximum_embedding_row}, "
                f"article embedding 행 수="
                f"{article_embedding_row_count}"
            )
    ...
    # STEP 10-4-8. Validation-origin event z(E) 생성
    # Event에 현재 여러 기사가 속하더라도 평균내지 않는다.
    # z(E) = Event가 생성될 당시 최초 기사의 e5 embedding 
    validation_event_embedding_vectors: list[np.ndarray] = []
    validation_event_embedding_rows: list[dict[str, int]] = []

    for event_embedding_row, row in enumerate(
        validation_event_embedding_input.iter_rows(named=True)
    ): 
        event_id = int(row["event_id"])
        first_article_id = int(row["first_article_id"])
        event_article_count = int(row["event_article_count"])
        ...
        # 최초 기사의 E5 embedding을 그대로 z(E)로 사용
        event_embedding = np.asarray(
            article_embeddings[embedding_row], dtype = np.float32,
        ).copy()

        # NaN / Inf 검사
        if not np.isfinite(event_embedding).all():
            raise ValueError(
                "Validation Event Embedding에 "
                "NaN 또는 Inf가 있습니다. "
                f"event_id={event_id}, "
                f"first_article_id={first_article_id}"
            )
        validation_event_embedding_vectors.append(
            event_embedding
        )

        validation_event_embedding_rows.append({
            "event_embedding_row": event_embedding_row, 
            "event_id": event_id, 
            "first_article_id": first_article_id, 
            "event_article_count": event_article_count
        })

    # STEP 10-4-9. Validation event embedding numpy 배열 생성
    if validation_event_embedding_vectors:
        validation_event_embeddings = \
        np.stack(validation_event_embedding_vectors, axis=0).astype(
            np.float32, copy=False,
        )        
    else:
        # validation 신규 event가 0개인 경우에도 안전하게 읽을 수 있도록 빈 2차원 배열 생성
        validation_event_embeddings = np.empty((0, article_embedding_dimension), dtype = np.float32)

    # STEP 10-4-10. Event embedding row mapping df 생성
    if validation_event_embedding_rows:
        validation_event_embedding_input_df = pl.DataFrame(validation_event_embedding_rows).\
        with_columns([
            pl.col("event_embedding_row").cast(pl.Int64),
            pl.col("event_id").cast(pl.Int64),
            pl.col("first_article_id").cast(pl.Int64),
            pl.col("event_article_count").cast(pl.Int64),
        ]).sort("event_embedding_row")
    else:
        validation_event_embedding_input_df = pl.DataFrame(
            schema={
                "event_embedding_row":pl.Int64,
                "event_id": pl.Int64,
                "first_article_id": pl.Int64,
                "event_article_count": pl.Int64,
            }
        )

    # STEP 10-4-11. 생성된 event 수 정합성 검사 
    if validation_event_embedding_input_df.height != validation_event_count:
        raise ValueError(
            "Validation-origin Event 수와 "
            "Event Embedding Input 행 수가 다릅니다. "
            f"event_count={validation_event_count}, "
            f"embedding_input_count="
            f"{validation_event_embedding_input_df.height}"
        )

    if (
        validation_event_embeddings.shape[0]
        != validation_event_count
    ):
        raise ValueError(
            "Validation-origin Event 수와 "
            "Event Embedding 배열 행 수가 다릅니다. "
            f"event_count={validation_event_count}, "
            f"embedding_array_count="
            f"{validation_event_embeddings.shape[0]}"
        )

    # STEP 10-4-12. 최종 파일 저장
    validation_event_embedding_input_df.write_parquet(
        config.VALIDATION_EVENT_EMBEDDING_INPUT_PATH,
        compression="zstd",
    )

    np.save(
        config.VALIDATION_EVENT_EMBEDDINGS_PATH,
        validation_event_embeddings,
    )
    ...
    return {
        ...
        "validation_origin_event_count": int(
            validation_event_count
        ),

        "validation_event_embedding_shape": [
            int(
                validation_event_embeddings.shape[0]
            ),
            int(
                validation_event_embeddings.shape[1]
            ),
        ],

        "validation_event_embedding_dtype": str(
            validation_event_embeddings.dtype
        ),
        ...
        "validation_event_embedding_input_path": str(
            config.VALIDATION_EVENT_EMBEDDING_INPUT_PATH
        ),

        "validation_event_embeddings_path": str(
            config.VALIDATION_EVENT_EMBEDDINGS_PATH
        ),
    }
...
def build_validation() -> dict[str, Any]:
    ...
    # Validation-origin event의 생성 시점 고정 z(E) 생성
    validation_event_embedding_result = (
        build_validation_event_embeddings()
    )
    ...
    return {
        ...
        "event_embeddings_result": validation_event_embedding_result,
    }
